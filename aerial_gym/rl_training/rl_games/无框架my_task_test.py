...
def test_navigation_task(vae_model_path, rl_model_path, input_dim, output_dim):
    ...
    # 创建环境
    rl_task_env = task_registry.make_task("my_task", headless=False, num_envs=2, use_warp=False)
    rl_task_env.reset()
    ...
    with torch.no_grad():
        ...
        for i in range(10000):
            ...
            # 前向传播
            actions, _, _ = rl_model(observations)  # 四维的动作包含xyz方向的速度和偏航角

            # 执行动作
            rl_task_env.step(actions.to(device))
            # 打印信息
            if i % 100 == 0:
...
if __name__ == "__main__":
    # VAE模型与RL模型应该保持一致
    vae_model_path = "/home/niu/workspaces/aerial_gym_simulator/aerial_gym/agent_encoder/weights/520_beta_3_LD_64_epoch_10.pth"
    rl_model_path = "/home/niu/workspaces/aerial_gym_simulator/aerial_gym/rl_training/rl_games/runs/520_3_10/nn/gen_ppo.pth"
